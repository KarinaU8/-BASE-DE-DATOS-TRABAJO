...
-- Inserción de datos (clientes, vehículos, técnicos y mantenimientos)
...
(7_ 'Hugo', 'Ramírez', 'Chapa y Pintura', '955777999', 'hugoramirez@email.com'),
...
-- Actualizar datos: cambio de teléfono de María González
UPDATE Clientes
...
-- En esta parte estoy practicando los diferentes tipos de JOIN
-- para entender cómo se relacionan las tablas y qué tipo de datos devuelve cada uno.
...
-- Aquí quiero ver cada mantenimiento con su cliente, vehículo y técnico
SELECT
    c.nombres + ' ' + c.apellidos AS cliente,
    v.placa,
    m.tipo_mantenimiento,
...
-- LEFT JOIN
-- Muestra todos los clientes, incluso si no tienen vehículos registrados
SELECT 
    c.nombres,
    c.apellidos,
    v.placa,
...
-- RIGHT JOIN
-- Muestra todos los vehículos, aunque no tengan un cliente asignado (poco común, pero útil para probar)
SELECT 
    v.placa,
    v.marca,
    c.nombres AS nombre_cliente,
...
-- FULL OUTER JOIN
-- Une ambas tablas mostrando todo, tengan o no coincidencias
SELECTD
...
-- CROSS JOIN
-- Multiplica los registros de ambas tablas (combinación total)
-- Solo lo uso con pocas filas porque genera muchos resultados
SELECT 
    c.nombres AS cliente,
    v.marca AS vehiculo
FROM Clientes c
...
-- Ejemplo final práctico combinando JOIN y filtros
-- Quiero ver los mantenimientos pendientes junto con el cliente y técnico asignado
SELECT
    m.codigo_mantenimiento,
    c.nombres + ' ' + c.apellidos AS cliente,
    v.placa,
...
-- En esta parte estoy practicando las Funciones Agregadas (SUM, COUNT, AVG, MAX, MIN)
-- junto con las cláusulas GROUP BY y HAVING para analizar y resumir los datos.

-- COUNT: Contar el número total de técnicos en el taller
SELECT COUNT(codigo_tecnico) AS total_tecnicos
FROM Tecnicos;

-- COUNT y GROUP BY: Contar cuántos vehículos tiene cada cliente
-- (Esto se puede hacer con subconsulta, como antes, o con JOIN y GROUP BY)
SELECTt
    c.nombres,
    c.apellidos,
...
-- SUM: No tengo una columna de "costo" para sumar, pero como ejemplo
-- podríamos sumar todos los años de los vehículos (aunque no tenga mucho sentido práctico)
SELECT SUM(anio) AS suma_de_anios
FROM Vehiculos;

-- AVG, MAX, MIN y GROUP BY: Analizar los años de los vehículos por marca
SELECT 
    marca,
    AVG(CAST(anio AS FLOAT)) AS anio_promedio, -- CAST a FLOAT para un promedio decimal
...
-- GROUP BY en múltiples columnas: Ver cuántos mantenimientos ha hecho cada técnico por tipo
SELECT 
    t.nombres + ' ' + t.apellidos AS tecnico,
    m.tipo_mantenimiento,
    COUNT(*) AS total
...
-- HAVING: Filtrar los resultados de un GROUP BY
-- Mostrar solo las marcas de vehículos que tienen 2 o más unidades registradas
SELECT 
    marca, 
    COUNT(*) AS total_unidades
FROM Vehiculos
...
-- HAVING y GROUP BY: Mostrar solo los técnicos que tienen 2 o más mantenimientos asignados
SELECT 
    t.nombres + ' ' + t.apellidos AS tecnico,
...
-- En esta parte estoy practicando subconsultas con EXISTS y funciones de fecha, cadena y conversión.

-- Subconsulta con EXISTS
-- Mostrar solo los clientes que SÍ tienen al menos un vehículo registrado.
-- EXISTS es eficiente porque se detiene en cuanto encuentra la primera coincidencia.
...
-- GETDATE: Obtener la fecha y hora actual del sistema
SELECT GETDATE() AS fecha_actual;

-- DAY, MONTH, YEAR: Extraer partes de una fecha
-- Ver el mes y año de los mantenimientos programados
SELECT 
    descripcion, 
    fecha_programada,
...
-- DATEADD: Sumar a una fecha
-- Calcular la fecha de vencimiento de una revisión (ejemplo 30 dias despues de la programada)
SELECT 
    descripcion, 
    fecha_programada,
...
-- Calcular cuántos días faltan desde hoy hasta la fecha programada del mantenimiento
SELECTs
    descripcion,
    fecha_programada,
...
-- CONCAT: Combinar cadenas (más seguro que '+' si hay nulos)
-- Unir nombre y correo del cliente
SELECT CONCAT(nombres, ' (', correo, ')') AS contacto_cliente
FROM Clientes;

-- LEN: Medir la longitud de una cadena
-- Ver la longitud de las placas de los vehículos
...
-- UPPER y LOWER: Cambiar a mayúsculas y minúsculas
-- Mostrar la marca en mayúsculas y el modelo en minúsculas
SELECT 
    UPPER(marca) AS marca_mayuscula,
    LOWER(modelo) AS modelo_minuscula
FROM Vehiculos;

-- LTRIM y RTRIM: Limpiar espacios en blanco al inicio o al final
-- (Vamos a simular datos con espacios para el ejemplo)
SELECT 
    RTRIM(LTRIM('   Juan Pérez   ')) AS nombre_limpio;

-- Funciones de Conversión de Datos

-- CONVERT: Convertir tipos de datos (similar a CAST pero con formato)
-- Mostrar la fecha programada en formato DD/MM/YYYY 
SELECTs
    descripcion, 
    CONVERT(NVARCHAR(10), fecha_programada, 103) AS fecha_formateada
FROM Mantenimientos;

-- Convertir el año (INT) a texto (NVARCHAR) para concatenarlo
SELECT 
    placa,
